...
Modelo Preditivo: Um modelo de séries temporais que prevê com precisão a demanda futura de itens.

Ferramenta de Recomendação: Uma aplicação web interativa que traduz a previsão em uma recomendação de pedido de estoque, com base em metas de negócio (Nível de Serviço).
...
parte_1_modelo_preditivo.ipynb: Notebook Jupyter que detalha a construção do modelo. Inclui:
...
Selecionar uma Loja e um Item específico.

Definir um Nível de Serviço desejado (ex: 95%).

Receber uma recomendação clara de "Quantas unidades pedir" para o próximo período.

Visualizar a previsão de demanda futura para o item selecionado.

Ver em detalhes como o cálculo foi feito (Demanda Prevista + Estoque de Segurança).
...
$$\text{Quantidade de Pedido (Q)} = \text{Demanda Prevista (D)} + \text{Estoque de Segurança (SS)}
$$Onde:
